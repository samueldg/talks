...
```bash
python -m unittest tests/test_0_unittest.py
```

Pytest is compatible! And has better output

```bash
pytest tests/test_0_unittest.py
```

From now on, I'll be using `-v` (`--verbose`) so the output is clearer.

```bash
pytest -v tests/test_0_unittest.py
```

First convert the simplest test to a function:

```bash
pytest -v tests/test_1_function.py
```

`self.assertRaises` -> `pytest.raises`

```bash
pytest -v tests/test_2_raises.py
```

Multiple tests in one:

```bash
pytest -v tests/test_3_parametrization.py
```

Add fixture:

```bash
pytest -v tests/test_4_fixtures.py
```

What if I don't want this to load every time?

```bash
pytest -v tests/test_5_fixture_scopes.py
```

Wait, why am I not seeing the `print()` output?

```bash
pytest -v -s tests/test_5_fixture_scopes.py
...
```bash
pytest -v -s tests/test_6_fixture_context.py
```

Builtin fixtures:

```bash
pytest -v -s tests/test_7_builtin_fixtures.py
# Get the full list: 
pytest --fixtures
```

```bash
pytest -v -s tests/test_8_expected_failures.py
# Go fix the bug!
pytest -v -s tests/test_8_expected_failures.py
```
...
```bash
pytest -v tests/test_4_fixtures.py -k test_get_second_element_for_sho
```

Marks:

```bash
pytest -v tests/test_9_custom_marks.py  -m fast
pytest -v tests/test_9_custom_marks.py  -m slow
```
